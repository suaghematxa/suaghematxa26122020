...
const nsxcontrol = require("../../models/nsx");
...
if(req.isAuthenticated()){
    nsxcontrol.find()
      ...
      .then(docs => {
        ...
        res.render('backend/nsx/nsx-all',{nsxs:nsxs,layout:'layouts/layoutsadmin'})
      })
      .catch(err => {
        console.log(err);
        res.status(500).json({
...
exports.nsxs_add_nsx=(req,res,next)=>{
  res.render('backend/nsx/nsx-create');
    }
...
exports.nsxs_get_nsx = (req, res, next) => {
  ...
  nsxcontrol.findById(id)
    ...
    .then(doc => {
      ...
      res.render('backend/nsx/nsx-detail',{nsx:doc,layout:'layouts/layoutsadmin'});
    })
    .catch(err => {
      console.log(err);
...
exports.nsxs_get_nsx_edit = (req, res, next) => {
  ...
  nsxcontrol.findById(id)
    ...
    .then(doc => {
      console.log("From database", doc);
      res.render('backend/nsx/nsx-detail-edit',{nsx:doc,layout:'layouts/layoutsadmin'});
    })
    .catch(err => {
      console.log(err);
      res.status(500).json({ error: err });
